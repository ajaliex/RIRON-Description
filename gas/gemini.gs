...
【評価基準】
- ユーザーは実際の税理士試験に向けて「条文の暗記」を行っています
- したがって、提供された論点データ（条文内容）の言い回しやキーワードを、【正確に暗記し、一語一句踏襲できているか】を厳格に評価してください
- 記載の大枠が合っていても、法律用語や条文特有の言い回しが不正確な場合は「不足・改善点」として厳しく指摘してください

【フィードバックの作成ルール】
- 「模範解答の要点」を作成する際は、一般的な表現に要約せず、必ず【論点の正確な内容】として与えられたデータ（条文）をそのまま忠実に引用して提示してください。これによりユーザー自身の暗記箇所の抜け漏れをチェックさせます
